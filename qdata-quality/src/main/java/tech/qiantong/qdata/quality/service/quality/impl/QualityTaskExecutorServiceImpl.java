...
//    private QualityRuleService qualityRuleService;
    ...
    public void executeQualityTask(String taskId,String key) {
        ...
        for (Map.Entry<Long, List<Long>> entry : groupIdsByDatasourceId.entrySet()) {
            ...
            if (daDatasourceById == null){
                ...
                taskFailed = true;
                failureMessages.add("Datasource not found: " + datasourceId);
                continue;
            }
            ...
            try {
                ...
                if (!dbQuery.valid()) {
                    ...
                    taskFailed = true;
                    failureMessages.add("Datasource connection failed: " + daDatasourceById.getDatasourceName());
                    continue;
                }
            }catch (Exception e){
                ...
                logger.log(MessageUtils.messageEn("quality.log.rule.skip.by.name", daDatasourceById.getDatasourceName(), daDatasourceById.getId()));
                taskFailed = true;
                failureMessages.add("Datasource connection error: " + daDatasourceById.getDatasourceName());
                continue;
            }
            ...
            for (DppQualityTaskEvaluateDO dppQualityTaskEvaluateDO : dppQualityTaskEvaluateList) {
                ...
                if(CollectionUtils.isEmpty(tableColumns)){
                    ...
                    taskFailed = true;
                    failureMessages.add("Unable to load columns for rule " + qualityRuleEntity.getId());
                    continue;
                }
                ...
                try {
                    ...
                    QualityCheckResult result = ruleExecutorTask.call();
                    if (StringUtils.isNotEmpty(result.getErrorMessage())) {
                        taskFailed = true;
                        failureMessages.add("Rule " + qualityRuleEntity.getId() + ": " + result.getErrorMessage());
                    }
                }catch (Exception e){
                    logger.log(MessageUtils.messageEn("quality.log.rule.execute.fail"));
                    logger.log(e.getMessage());
                    taskFailed = true;
                    failureMessages.add("Rule " + qualityRuleEntity.getId() + ": " + e.getMessage());
                    continue;
                }
        ...
        updateQualityLogSummary(taskLogId);
        updateQualityLog(
                taskLogId,
                taskFailed ? "1" : "0",
                taskFailed ? String.join("; ", failureMessages) : null
        );
        logger.log(MessageUtils.messageEn("quality.log.task.end"));
        // After the task is completed, close the logger and release resources
        logger.close();
    ...
    private void updateQualityLogSummary(Long taskLogId) {
        Map<String, Object> summary =
                iDppEvaluateLogService.sumTotalAndProblemTotalByTaskLogId(String.valueOf(taskLogId));
        long total = summary.get("total") == null ? 0L : ((Number) summary.get("total")).longValue();
        long problemTotal = summary.get("problemTotal") == null ? 0L
                : ((Number) summary.get("problemTotal")).longValue();
        long score = 0L;
        if (total > 0) {
        ...
        vo.setId(taskLogId);
        vo.setScore(score);
        vo.setProblemData(problemTotal);
    ...
    public void updateQualityLog(Long id, String successFlag) {
        updateQualityLog(id, successFlag, null);
    }

    public void updateQualityLog(Long id, String successFlag, String remark) {
        DppQualityLogSaveReqVO vo = new DppQualityLogSaveReqVO();
        vo.setId(id);
        vo.setSuccessFlag(successFlag);
        vo.setEndTime(DateUtil.date());
        if (StringUtils.isNotEmpty(remark)) {
            vo.setRemark(remark.length() > 256 ? remark.substring(0, 256) : remark);
        }
