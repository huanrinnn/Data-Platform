...
@Component("CONDITIONAL_FIELD_VALIDATION")
public class ConditionalFieldValidationGenerator implements QualitySqlGenerator {
    @Override
    public String generateSql(QualityRuleEntity rule) {
        ComponentItem componentItem = componentItem(rule);
        return componentItem.generateConditionalFieldValidationSql(rule);
    }

    @Override
    public String generateErrorSql(QualityRuleEntity rule) {
        ComponentItem componentItem = componentItem(rule);
        return componentItem.generateConditionalFieldValidationErrorSql(rule);
    }

    @Override
    public String generateValidDataSql(QualityRuleEntity rule, int limit, int offset) {
        ComponentItem componentItem = componentItem(rule);
        return componentItem.generateConditionalFieldValidationValidDataSql(rule, limit, offset);
    }

    private ComponentItem componentItem(QualityRuleEntity rule) {
