...
public class DppEvaluateLogServiceImpl  extends ServiceImpl<DppEvaluateLogMapper, DppEvaluateLogDO> implements IDppEvaluateLogService {
    ...
    public Map<String, Object> sumTotalAndProblemTotalByTaskLogId(String taskLogId) {
        List<DppEvaluateLogDO> logs = list(new QueryWrapper<DppEvaluateLogDO>()
                .eq("task_log_id", taskLogId));
        long total = logs.stream()
                .filter(log -> log.getTotal() != null)
                .mapToLong(DppEvaluateLogDO::getTotal)
                .sum();
        long problemTotal = logs.stream()
                .filter(log -> log.getProblemTotal() != null && log.getProblemTotal() >= 0)
                .mapToLong(DppEvaluateLogDO::getProblemTotal)
                .sum();
        Map<String, Object> result = new HashMap<>();
        result.put("total", total);
        result.put("problemTotal", problemTotal);
        return result;
    }
