...
public interface ComponentItem extends QualityFragSql {
    ...
    default String generateCompositeUniquenessValidationSql(QualityRuleEntity rule) {
        ...
        String nonNullClause = columns.stream()
        ...
        query.append(") AS totalCount,")
                ...
                .append("   FROM ")
                .append(table);
        query.append(" WHERE ").append(nonNullClause);
        if (StringUtils.isNotEmpty(whereClause)) {
    ...
    default String generateCompositeUniquenessValidationErrorSql(QualityRuleEntity rule) {
        ...
        query.append("SELECT * FROM ").append(table)
                .append(" WHERE ").append(nonNullClause).append(" AND ");
    ...
    /** Date/time format validation: returns error count and total count. */
    default String generateDateFormatValidationSql(QualityRuleEntity rule) {
        return generateSql(rule, dateFormatErrorFrag(rule));
    }

    /** Date/time format validation error rows. */
    default String generateDateFormatValidationErrorSql(QualityRuleEntity rule) {
        return generateDataSql(rule, dateFormatErrorFrag(rule));
    }

    /** Date/time format validation valid rows. */
    default String generateDateFormatValidationValidDataSql(QualityRuleEntity rule, int limit, int offset) {
        return addPagination(generateDataSql(rule, dateFormatValidFrag(rule)), limit, offset);
    }

    default String dateFormatErrorFrag(QualityRuleEntity rule) {
        String frag = fragDateFormat(rule);
        if (ignoreNullValue(rule)) {
            return String.format("%s is not null and not (%s)", rule.getRuleColumn(), frag);
        }
        return String.format("%s is null or not (%s)", rule.getRuleColumn(), frag);
    }

    default String dateFormatValidFrag(QualityRuleEntity rule) {
        String frag = fragDateFormat(rule);
        if (ignoreNullValue(rule)) {
    ...
    default String generateLogicCompareValidationSql(QualityRuleEntity rule) {
        ...
        cn.hutool.core.lang.Assert.isFalse(conditions == null || conditions.isEmpty(), "At least one time comparison condition must be configured");
