...
public interface QualityFragSql {
    ...
    default String regex(String column, String regex) {
        Assert.notBlank(regex, "Regular expression must not be blank");
        return String.format("REGEXP_LIKE(%s, '%s')", column, regex.replace("'", "''"));
    }
    ...
    default String fragLength(QualityRuleEntity rule) {
        ...
        Assert.isFalse(tmp.isEmpty(), "At least one length boundary must be configured");
    ...
    default String fragDecimalPrecision(QualityRuleEntity rule) {
        ...
        Assert.isTrue(scale >= 0, "Decimal precision must not be negative");
    ...
    default String fragEnum(QualityRuleEntity rule) {
        ...
        Assert.isFalse(CollectionUtils.isEmpty(values), "At least one enumeration value must be configured");
        boolean ignoreCase = SqlBuilderUtils.parseBoolean(rule.getConfig().get("ignoreCase"));
        Set<String> escapedValues = values.stream()
                .filter(Objects::nonNull)
                .map(value -> value.replace("'", "''"))
                .collect(Collectors.toSet());
        Assert.isFalse(CollectionUtils.isEmpty(escapedValues), "At least one non-null enumeration value must be configured");
        String frag;
        if (ignoreCase) {
            Set<String> collect = escapedValues.stream().map(String::toUpperCase).collect(Collectors.toSet());
            frag = String.format("UPPER(%s) in('%s')", column, String.join("','", collect));
        } else {
            frag = String.format("%s in('%s')", column, String.join("','", escapedValues));
        }
        return frag;
    }
    ...
    default String fragFieldCompleteness(List<String> columns, int fillStrategy) {
        Assert.isFalse(CollectionUtils.isEmpty(columns), "At least one field must be configured");
        String frag;
        if (fillStrategy == 1) {
            frag = columns.stream()
    ...
    /**
     * Date/time format validation fragment. Dialects may override this when
     * they need strict date parsing in addition to shape validation.
     */
    default String fragDateFormat(QualityRuleEntity rule) {
        String column = rule.getRuleColumn();
        ...
        return regex("CAST(" + column + " AS CHAR)", pattern);
    }
