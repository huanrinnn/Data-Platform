...
//            }
    ...
    public Map<String, Object> getColumnData(JSONObject jsonObject) {
        ...
        List<DbColumn> previewColumns = CollectionUtils.isNotEmpty(daAssetColumns) ? daAssetColumns : columns;
        List<Map<String, Object>> columnTable = new ArrayList<>();
        for (DbColumn column : previewColumns) {
...
//        createDaAssetProjectRel(daAsset);
    ...
    private void createDaAssetColumnNew(DaAssetSaveReqVO daAsset) {
        ...
        if (daAsset.getTableId() != null) {
            ...
            if (CollectionUtils.isNotEmpty(mcColumnRespDTOList)) {
                daAssetColumnDOS = mcColumnRespDTOList.stream()
                        .map(mcColumnRespDTO -> new DaAssetColumnDO(mcColumnRespDTO))
                        .collect(Collectors.toList());
            }
        }
        if (CollectionUtils.isEmpty(daAssetColumnDOS) && daAsset.getDatasourceId() != null && StringUtils.isNotBlank(daAsset.getTableName())) {
            DaDatasourceDO daDatasourceDO = daDatasourceMapper.selectById(daAsset.getDatasourceId());
            if (daDatasourceDO != null) {
                DbQueryProperty dbQueryProperty = new DbQueryProperty(daDatasourceDO.getDatasourceType(), daDatasourceDO.getIp(), daDatasourceDO.getPort(), daDatasourceDO.getDatasourceConfig());
                DbQuery dbQuery = dataSourceFactory.createDbQuery(dbQueryProperty);
                try {
                    if (dbQuery.valid()) {
                        List<DbColumn> tableColumns = dbQuery.getTableColumns(dbQueryProperty, daAsset.getTableName());
                        if (CollectionUtils.isNotEmpty(tableColumns)) {
                            daAssetColumnDOS = tableColumns.stream()
                                    .map(column -> {
                                        DaAssetColumnDO assetColumn = new DaAssetColumnDO();
                                        assetColumn.setColumnName(column.getColName());
                                        assetColumn.setColumnComment(column.getColComment());
                                        assetColumn.setColumnType(column.getDataType());
                                        assetColumn.setColumnLength(StringUtils.isNotBlank(column.getDataLength()) ? Long.valueOf(column.getDataLength()) : null);
                                        assetColumn.setColumnScale(StringUtils.isNotBlank(column.getDataScale()) ? Long.valueOf(column.getDataScale()) : null);
                                        assetColumn.setNullableFlag(Boolean.TRUE.equals(column.getNullable()) ? "0" : "1");
                                        assetColumn.setPkFlag(Boolean.TRUE.equals(column.getColKey()) ? "1" : "0");
                                        return assetColumn;
                                    })
                                    .collect(Collectors.toList());
                        }
        ...
        if (CollectionUtils.isEmpty(daAssetColumnDOS)) {
            daAsset.setFieldCount(0L);
        } else {
    ...
    public List<Map<String, Object>> dataMaskings(Long assetId, List<Map<String, Object>> data, Long userId, String scene) {
        ...
        for (Map<String, Object> row : data) {
            ...
            for (Map.Entry<String, Object> e : row.entrySet()) {
                String key = e.getKey();
                Object val = e.getValue();
                Object maskType = mt.getOrDefault(key, 1);
                if(maskType.toString().equals("1")){
                    masked.put(key, val);
                }else if(maskType.toString().equals("2")){
