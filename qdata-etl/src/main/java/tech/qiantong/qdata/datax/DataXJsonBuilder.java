...
public final class DataXJsonBuilder {
    ...
    private static Map<String, Object> buildParameter(Map<String, Object> nodeJsonMap,
        ...
        String tableName = String.valueOf(nodeJsonMap.get(tableKey));
        ...
        if (object != null) {
            if (object.toString().equals("3")) {
                ...
                // Full sync uses truncate + implicit insert.
                Object existingPreSql = parameter.get("preSql");
                List<Object> preSqlList = new ArrayList<>();
                preSqlList.add("truncate table " + tableName);
                if (existingPreSql instanceof Collection) {
                    preSqlList.addAll((Collection<?>) existingPreSql);
                } else if (existingPreSql != null) {
                    preSqlList.add(existingPreSql);
                }
                parameter.put("preSql", preSqlList);
            } else if (object.toString().equals("2")) {
                // Append mode relies on the DataX default insert behavior.
            }
        }
