...
post_boot_first_run() {
  ...
  # Keep this idempotent so a retry or partial init does not break startup.
  set +e
...
EOF
  /home/dmdba/dmdb/bin/disql SYSDBA/${SYSDBA_PWD}@localhost:${PORT_NUM} <<EOF >> "$INIT_LOG" 2>&1
create user "WATER_TP" identified by "${QDATA_PWD}" hash with SHA512 salt;
grant "PUBLIC","SOI","DBA" to "WATER_TP";
COMMIT;
EOF
  set -e
