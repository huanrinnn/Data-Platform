...
function createDefaultQualityTaskForm() {
  return {
    assetFlag: "1",
    assetId: "",
    taskName: "",
...
function open(data, row) {
  handleClose();
  Object.assign(form.value, createDefaultQualityTaskForm());
...
async function submitForm() {
  ...
  try {
    ...
    console.warn("Failed to submit asset quality task", err);
    ElMessage.error(err?.message || td('dpp.asset.detail.quality.submitFailed'));
  } finally {
    loadingInstance.value = false;
  }
}
