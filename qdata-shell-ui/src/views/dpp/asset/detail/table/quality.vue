...
function openQualityDialog(row) {
    ...
    qualityDialog.value.open(
        ...
        row?.id ? JSON.parse(JSON.stringify(row)) : undefined
    );
...
const buildConfiguredSummary = (rules = []) => {
    const summaryMap = new Map();
    rules.forEach((item) => {
        const key = item?.dimensionType ?? '-1';
        summaryMap.set(key, (summaryMap.get(key) || 0) + 1);
    });

    return att_rule_audit_q_dimension.value.map((dim) => ({
        dimensionType: dim.value,
        succesTotal: summaryMap.get(dim.value) || 0,
        proportion: 0,
        trendType: '-1',
    }));
};

const buildConfiguredRuleList = (task = {}) => {
    const rules = Array.isArray(task?.dppQualityTaskEvaluateRespVOS)
        ? task.dppQualityTaskEvaluateRespVOS
        : [];
    const objectMap = new Map(
        (Array.isArray(task?.dppQualityTaskObjSaveReqVO) ? task.dppQualityTaskObjSaveReqVO : []).map((item) => [
            `${item.datasourceId}_${item.tableName}`,
            item,
        ])
    );

    return rules.map((item) => {
        const obj = objectMap.get(`${item.datasourceId}_${item.tableName}`) || {};
        return {
            ...item,
            datasourceName: obj.name || item.objName || item.tableName || '-',
            columnLabel: item.evaColumn || '-',
            problemTotal: 0,
            proportion: 0,
        };
    });
...
let taskData = ref({})
const assetId = computed(() => props.form1?.id || route.query.id);
const fetchData = async () => {
    loading.value = true;
    try {
        if (!assetId.value) {
            loadChartWithData({ title: [], value: [] });
            return;
        }

        const res = await getQualityTaskAsset({ assetId: assetId.value });
        if (res.data) {
            taskData.value = res.data
            console.log("🚀 ~ fetchData ~ res.data:", taskData.value)
            console.log("🚀 ~ fetchData ~  taskData.value:", taskData.value.status)
            summaryList.value = buildConfiguredSummary(res.data?.dppQualityTaskEvaluateRespVOS || [])
            ruleList.value = buildConfiguredRuleList(res.data)
        ...
        console.warn("Failed to load asset quality", err);
        loadChartWithData({ title: [], value: [] });
    } finally {
        loading.value = false;
    }
