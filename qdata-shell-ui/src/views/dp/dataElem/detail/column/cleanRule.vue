...
function RuleSelectorconfirm(obj, mode) {
    ...
    if (!ruleId) {
        proxy.$message.warning(td('dp.dataElem.detail.ruleIdRequired', '规则ID不能为空，请重新选择规则'));
        return;
    }
    let api = obj?.id ? putDpDataElemRuleRel : dpDataElemRuleRel;
    loading.value = true;
    api({ ...obj, rule: obj.ruleConfig, dataElemId: props.dataElemId, type: 2, ruleId, ruleDescription: obj.ruleDesc, dimensionType: obj.parentName }).then((res) => {
        if (res.code == 200) {
