            ...
            <el-row :gutter="15" class="btn-style">
              <el-col :span="1.5">
                <el-button
                  ...
                  plain
                  @click="handleAdd"
                  v-hasPermi="['att:auditRule:add']"
                  @mousedown="(e) => e.preventDefault()"
                >
                  <i class="iconfont-mini icon-xincheng mr5"></i>{{ td('common.button.add') }}
                </el-button>
              </el-col>
              <!--                            <el-col :span="1.5">-->
      ...
      <el-form
       ...
       :label-position="labelPosition">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="执行策略" prop="strategyKey" :label-position="labelPosition">
              <el-select v-model="form.strategyKey" placeholder="请选择通用执行策略" style="width: 100%">
                <el-option
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="默认配置" prop="ruleConfig" :label-position="labelPosition">
              <el-input
                v-model="form.ruleConfig"
                type="textarea"
                :rows="5"
                placeholder='请输入 JSON，例如：{"useRegexFlag":1,"regex":"^1[3-9][0-9]{9}$","ignoreNullValue":"0"}'
              />
            </el-form-item>
          </el-col>
        </el-row>
...
const strategyOptions = [
  { value: "NOT_NULL_VALIDATION", label: "字段非空校验" },
  { value: "CHARACTER_VALIDATION", label: "字符或正则校验" },
  { value: "COMPOSITE_UNIQUENESS_VALIDATION", label: "字段唯一性校验" },
  { value: "DECIMAL_PRECISION_VALIDATION", label: "数值精度校验" },
  { value: "LENGTH_VALIDATION", label: "字段长度校验" },
  { value: "NUMERIC_RANGE_VALIDATION", label: "数值范围校验" },
  { value: "ENUM_VALIDATION", label: "枚举值校验" },
        ...
        validator: (rule, value, callback) => {
          ...
          try {
            ...
            if (!parsed || Array.isArray(parsed) || typeof parsed !== "object") {
              callback(new Error("默认配置必须是 JSON 对象"));
              return;
            }
            callback();
          } catch {
            callback(new Error("默认配置不是有效的 JSON"));
          }
        },
        trigger: "blur",
      },
