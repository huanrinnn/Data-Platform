...
<template>
    <el-dialog v-model="visible" :title="td('da.qualityTaskLog.problemDataEdit.title')" class="medium-dialog" @close="handleClose" destroy-on-close>
        <el-form :model="formData" label-width="240px" :label-position="labelPosition">
            <el-form-item v-for="item in parsedFields" :key="item.key" :label="item.name" :label-position="labelPosition">
                <el-input v-model="formData[item.key]" :disabled="!item.editable" />
            </el-form-item>
        </el-form>
    ...
    try {
        ...
        const labelMap = (value && Array.isArray(value.evaColumns))
            ? value.evaColumns.reduce((map, col) => {
                map[normalizeFieldName(col.name)] = col.label || col.name
                return map
            }, {})
            : {}

        const editableFields = columnName
            ? columnName.split(',').map(n => normalizeFieldName(n.trim()))
            : []

        formData.value = {}
        parsedFields.value = Object.entries(json).map(([key, val]) => {
            const label = labelMap[normalizeFieldName(key)] || key

            formData.value[key] = val

            const isEditable = editableFields.includes(normalizeFieldName(key))
...
function handleok() {
    ...
    for (const field of parsedFields.value) {
        ...
        const newValue = formData.value[field.key]
        const originKey = Object.keys(originalJson).find(
            k => normalizeFieldName(k) === normalizeFieldName(field.label)
        ) || field.label
        const oldValue = originalJson[originKey]

        // Use field.label as key
        if (newValue != oldValue) {
            keyWordData[field.key] = newValue
        }
    }
    ...
    const result = Object.entries(formData.value).reduce((acc, [key, val]) => {
        const field = parsedFields.value.find(f => f.key === key)
        if (field) {
            acc[field.key] = val
        }
        return acc
