  ...
  >
    ...
    >
      <el-form ref="formRef" :model="form" :label-position="labelPosition">
        ...
        <el-row :gutter="20">
          <el-col :span="12" class="hasMsg" v-if="type != 3 || form.ruleType == 'TIME_ORDER_VALIDATION'">
            <el-form-item
             ...
             :label-position="labelPosition">
              <template v-if="!falg">
                <el-select
                  ...
                  filterable
                  clearable
                  :disabled="type == 2 && form.ruleType != 'TIME_ORDER_VALIDATION'"
...
function handleCardClick(data) {
  ...
  if (props.type == 3 && form.ruleType != "TIME_ORDER_VALIDATION") {
    form.tableName = props.tableName;
  }
  if (data?.ruleConfig) {
    try {
      const defaultRuleConfig =
        typeof data.ruleConfig === "string"
          ? JSON.parse(data.ruleConfig)
          : data.ruleConfig;
      if (defaultRuleConfig && typeof defaultRuleConfig === "object") {
        form.rule = { ...form.rule, ...defaultRuleConfig };
      }
    } catch {
      proxy.$message.warning("规则默认配置不是有效的 JSON，请联系规则管理员");
    }
  }
...
async function openDialog(record, index, fg) {
  ...
  if (index) {
    ...
    if (props.type == 2 && form.ruleType != "TIME_ORDER_VALIDATION") {
      form.tableName = props.tableName;
    }
    if (
      form.ruleType == "COMPOSITE_UNIQUENESS_VALIDATION" ||
