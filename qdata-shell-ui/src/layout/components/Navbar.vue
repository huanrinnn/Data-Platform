...
function projectIdChange(row, newValue) {
  ...
  if (userStore.projectId) {
    localStorage.setItem(shellStorageKey("qdataProjectId"), userStore.projectId);
...
const listProject = () => {
  ...
  const storedProjectId = Number(localStorage.getItem(shellStorageKey("qdataProjectId")));
      const storedProject = Number.isFinite(storedProjectId)
        ? projectOptions.value.find((item) => item.id === storedProjectId)
        : null;
      const selectedProject = storedProject || projectOptions.value[0];

      userStore.projectId = selectedProject ? selectedProject.id : null;
      userStore.projectCode = selectedProject ? selectedProject.code : "";

      if (selectedProject) {
  localStorage.setItem(shellStorageKey("qdataProjectId"), String(selectedProject.id));
        getRoutersDpp(selectedProject.id).then((res) => {
          // Update routing data in the store
          permissionStore.updateTopbarRoutes(res.data);
        });
      }
