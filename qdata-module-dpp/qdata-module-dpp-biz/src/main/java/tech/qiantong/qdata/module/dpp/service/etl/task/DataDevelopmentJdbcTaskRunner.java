...
public class DataDevelopmentJdbcTaskRunner {
    ...
    private Long createNodeInstance(DppEtlTaskDO task, DppEtlNodeRespVO node, DppEtlTaskInstanceDO instance) {
        ...
        try {
            ...
            DppEtlNodeInstanceDO nodeInstance = DppEtlNodeInstanceDO.builder()
                    .id(id)
                    .taskType(task.getType())
                    .name(node.getName())
                    .nodeType(String.valueOf(params.getOrDefault("taskType", "SQL")))
                    .nodeId(node.getId())
                    .nodeCode(node.getCode())
                    .nodeVersion(node.getVersion() == null ? 1 : node.getVersion().intValue())
                    ...
                    .priority(node.getPriority() == null ? String.valueOf(Priority.MEDIUM.getCode()) : node.getPriority())
                    .retryTimes(node.getFailRetryTimes() == null ? null : node.getFailRetryTimes().intValue())
                    .delayTime(node.getDelayTime() == null ? null : node.getDelayTime().intValue())
                    ...
                    .build();
            dppEtlNodeInstanceService.save(nodeInstance);
        } catch (Exception e) {
