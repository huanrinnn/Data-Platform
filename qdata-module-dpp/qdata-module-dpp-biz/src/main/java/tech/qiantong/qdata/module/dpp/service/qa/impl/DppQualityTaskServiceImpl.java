...
public class DppQualityTaskServiceImpl  extends ServiceImpl<DppQualityTaskMapper,DppQualityTaskDO> implements IDppQualityTaskService, DppQualityTaskApiService {
    ...
    public Long createDppQualityTask(DppQualityTaskSaveReqVO createReqVO) {
        fillQualityTaskDefaults(createReqVO);
        ...
        if(StringUtils.equals("1",assetFlag)){
            ...
            wrapper.selectAll(DppQualityTaskDO.class)
                    ...
                    .orderByDesc(DppQualityTaskDO::getCreateTime);
            List<DppQualityTaskDO> taskDO = dppQualityTaskMapper.selectList(wrapper);
            if(CollectionUtils.isNotEmpty(taskDO)){
                createReqVO.setId(taskDO.get(0).getId());
                DppQualityTaskDO qualityTaskDO = BeanUtils.toBean(createReqVO, DppQualityTaskDO.class);
                dppQualityTaskMapper.updateById(qualityTaskDO);
                dppQualityTaskEvaluateService.remove(Wrappers.lambdaQuery(DppQualityTaskEvaluateDO.class)
                        .eq(DppQualityTaskEvaluateDO::getTaskId, createReqVO.getId()));
                dppQualityTaskObjService.remove(Wrappers.lambdaQuery(DppQualityTaskObjDO.class)
                        .eq(DppQualityTaskObjDO::getTaskId, createReqVO.getId()));
                saveQualityTaskChildren(createReqVO, createReqVO.getId());
        ...
        saveQualityTaskChildren(createReqVO, dictType.getId());
        return dictType.getId();
    }

    private void fillQualityTaskDefaults(DppQualityTaskSaveReqVO task) {
        if (StringUtils.isBlank(task.getCatCode())) {
            task.setCatCode("-1");
        }
        ...
        if (StringUtils.isBlank(task.getWorkerGroup())) {
            task.setWorkerGroup("default");
        }
        if (StringUtils.isBlank(task.getAssetFlag())) {
            task.setAssetFlag("0");
        }
        if (task.getRetryTimes() == null) {
            task.setRetryTimes(0L);
        }
        if (task.getRetryInterval() == null) {
            task.setRetryInterval(0L);
        }
        if (task.getDelayTime() == null) {
            task.setDelayTime(0L);
        }
        if (task.getDppQualityTaskObjSaveReqVO() == null) {
            task.setDppQualityTaskObjSaveReqVO(Collections.emptyList());
        }
    }

    private void saveQualityTaskChildren(DppQualityTaskSaveReqVO createReqVO, Long taskId) {
        ...
        for (DppQualityTaskObjSaveReqVO qualityTaskObjSaveReqVO : dppQualityTaskObjSaveReqVO) {
            qualityTaskObjSaveReqVO.setTaskId(taskId);
        ...
        if (dppQualityTaskEvaluateSaveReqVO != null) {
            for (DppQualityTaskEvaluateSaveReqVO qualityTaskEvaluateSaveReqVO : dppQualityTaskEvaluateSaveReqVO) {
                ...
                if (dppQualityTaskObjSaveReqVO1 != null) {
                    qualityTaskEvaluateSaveReqVO.setTaskId(taskId);
    ...
    public int updateDppQualityTask(DppQualityTaskSaveReqVO updateReqVO) {
        ...
        if (dppQualityTaskEvaluateSaveReqVO == null || dppQualityTaskEvaluateSaveReqVO.isEmpty()) {
            dppQualityTaskEvaluateService.remove(Wrappers.lambdaQuery(DppQualityTaskEvaluateDO.class)
                    .eq(DppQualityTaskEvaluateDO::getTaskId, dictType.getId()));
        } else {
            List<Long> submittedEvaluateIds = dppQualityTaskEvaluateSaveReqVO.stream()
                    .map(DppQualityTaskEvaluateSaveReqVO::getId)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList());
            if (CollectionUtils.isEmpty(submittedEvaluateIds)) {
                dppQualityTaskEvaluateService.remove(Wrappers.lambdaQuery(DppQualityTaskEvaluateDO.class)
                        .eq(DppQualityTaskEvaluateDO::getTaskId, dictType.getId()));
            } else {
                dppQualityTaskEvaluateService.remove(Wrappers.lambdaQuery(DppQualityTaskEvaluateDO.class)
                        .eq(DppQualityTaskEvaluateDO::getTaskId, dictType.getId())
                        .notIn(DppQualityTaskEvaluateDO::getId, submittedEvaluateIds));
            }
            for (DppQualityTaskEvaluateSaveReqVO qualityTaskEvaluateSaveReqVO : dppQualityTaskEvaluateSaveReqVO) {
                DppQualityTaskObjSaveReqVO dppQualityTaskObjSaveReqVO1 = collect.get(qualityTaskEvaluateSaveReqVO.getDatasourceId() + qualityTaskEvaluateSaveReqVO.getTableName());
                if (dppQualityTaskObjSaveReqVO1 != null) {
    ...
    public DppQualityTaskRespVO getQualityTaskAsset(DppQualityTaskAssetReqVO dppQualityTaskAssetReqVO) {
        if (dppQualityTaskAssetReqVO == null || dppQualityTaskAssetReqVO.getAssetId() == null) {
            return null;
        }
        MPJLambdaWrapper<DppQualityTaskDO> wrapper = new MPJLambdaWrapper<>();
        wrapper.selectAll(DppQualityTaskDO.class)
                ...
                .orderByDesc(DppQualityTaskDO::getCreateTime);
        List<DppQualityTaskDO> taskList = dppQualityTaskMapper.selectList(wrapper);
        DppQualityTaskDO taskDO = CollectionUtils.isEmpty(taskList) ? null : taskList.get(0);
        if(taskDO == null){
            return null;
        }
...
//        String s = this.validateInputWithRegex(lists);
    ...
    public AjaxResult startDppQualityTask(Long id) {
        ...
        try {
            HttpUtils.ResponseObject responseObject = HttpUtils.sendPut(
                    url + "/runExecuteTask/" + id,
                    Collections.emptyMap(),
                    headers
            );
            if (responseObject == null) {
                return error(MessageUtils.messageWithFallback(
                        "dpp.error.task.execute.fail", "Task execution failed"));
            }
            ...
            if (body instanceof JSONObject) {
                ...
                return code != null && code == 200 ? success(msg) : error(msg);
            }
            return success();
        } catch (IOException e) {
            throw new RuntimeException(e);
