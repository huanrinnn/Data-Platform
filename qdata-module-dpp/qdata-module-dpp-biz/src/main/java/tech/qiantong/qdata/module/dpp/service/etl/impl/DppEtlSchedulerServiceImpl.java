...
public class DppEtlSchedulerServiceImpl  extends ServiceImpl<DppEtlSchedulerMapper,DppEtlSchedulerDO> implements IDppEtlSchedulerService {
    ...
    public DppEtlSchedulerDO getDppEtlSchedulerById(DppEtlSchedulerPageReqVO reqVO) {
        ...
        DppEtlSchedulerDO scheduler = dppEtlSchedulerMapper.selectOne(wrapper);
        if (scheduler == null && reqVO.getTaskId() != null && StringUtils.isNotBlank(reqVO.getTaskCode())) {
            MPJLambdaWrapper<DppEtlSchedulerDO> fallbackWrapper = new MPJLambdaWrapper<>();
            fallbackWrapper.selectAll(DppEtlSchedulerDO.class)
                    .eq(DppEtlSchedulerDO::getTaskId, reqVO.getTaskId());
            scheduler = dppEtlSchedulerMapper.selectOne(fallbackWrapper);
        }
        return scheduler;
    }
